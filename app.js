...
function can(id){var a=A[id];if(a=='all')return true;return ACC[a].indexOf(S.user.role)>=0}
...
$('#tLogin').onclick=function(){tabUI('login')};$('#tReg').onclick=function(){tabUI('register')};
$('#goBtn').onclick=function(){var em=$('#qEmail').value.trim().toLowerCase(),p=$('#qPass').value,ok=true;function E(i,m){$('#'+i).textContent=m;if(m)ok=false}E('eE',/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(em)?'':'ایمیل معتبر نیست.');E('eP',p.length<6?'رمز حداقل ۶ کاراکتر.':'');if(mode=='register'&&$('#qPass2').value!==p)E('eP2','تکرار رمز مطابقت ندارد.');if(!ok)return;var us=D('pp_users',[]);
hsh(p).then(function(h){
if(mode=='register'){var dup=false;for(var z=0;z<us.length;z++){if(us[z].email===em){dup=true;break}}if(dup){$('#eE').textContent='این ایمیل قبلاً ثبت شده.';return}var role=us.length?'viewer':'admin';var u={name:STR($('#qName').value,em.split('@')[0]),email:em,hash:h,role:role};us.push(u);SV('pp_users',us);localStorage.setItem('pp_session',em);start(u);toast(role=='admin'?'شما مدیر کل هستید 👑':'ثبت‌نام شدید؛ مدیر نقش شما را تعیین می‌کند.')}
else{var f=null;for(var i=0;i<us.length;i++){if(us[i].email===em&&us[i].hash===h){f=us[i];break}}if(!f){$('#eTop').textContent='ایمیل یا رمز اشتباه است.';return}localStorage.setItem('pp_session',em);start(f);toast('خوش آمدید '+f.name)}})};
...
function renderAside(){$('#aside').innerHTML=MENU.map(function(g){var ok=g[1].filter(function(i){return can(i[0])});return ok.length?'<div class="grp"><h4>'+g[0]+'</h4>'+ok.map(function(i){return'<button class="sitem" data-v="'+i[0]+'">'+i[1]+'</button>'}).join('')+'</div>':''}).join('');$$('#aside .sitem').forEach(function(b){b.onclick=function(){go(b.getAttribute('data-v'))}})}
var RENDER={};
...
var bs=D('pp_batches',[]),act=bs.filter(function(b){return b.st!=='done'}).length;
...
+'<div class="panel"><h3>دسترسی نقش شما</h3><div style="font-size:13px;line-height:2.2">بخش‌های مجاز نقش شما در منوی کنار نمایش داده شده است. 👑 مدیر کل همه بخش‌ها را می‌بیند.</div></div>'};
...
$('#bAdd').onclick=function(){var w=N0($('#bW').value),p=N0($('#bP').value);if(!(w&&p)){toast('وزن و قیمت را وارد کن.',1);return}var r={d:$('#bD').value,s:STR($('#bSup').value,'—'),m:$('#bM').value,w:w,p:p,t:N0($('#bT').value)};arr.push(r);SV('pp_buy',arr);
var inv=D('pp_inv',[]),f2=null;for(var z=0;z<inv.length;z++){if(inv[z].c==='raw'&&inv[z].n===r.m){f2=inv[z];break}}if(f2)f2.q=N0(f2.q)+w;else inv.push({c:'raw',n:r.m,q:w,u:'kg',m:1000});SV('pp_inv',inv);
var fin=D('pp_fin',[]);fin.push({d:r.d,t:'out',de:'خرید '+r.m+' — '+r.s,a:r.p+r.t});SV('pp_fin',fin);toast('خرید ثبت شد ✅');RENDER.buy()}};
RENDER.inv=function(){var cat='raw';
function draw(){var inv=D('pp_inv',[]);
var rows=inv.filter(function(i){return i.c===cat}).map(function(i){var low=N0(i.q)<N0(i.m);return'<tr><td>'+esc(i.n)+'</td><td dir="ltr">'+fmt(i.q)+'</td><td>'+esc(i.u)+'</td><td dir="ltr">'+fmt(i.m)+'</td><td class="'+(low?'badc':'okc')+'">'+(low?'کمبود!':'موجود')+'</td></tr>'}).join('');
$('#main').innerHTML=head('انبار سه‌بخشی','مواد اولیه / محصولات / قطعات')+'<div class="seg"><button data-c="raw" class="'+(cat=='raw'?'on':'')+'">🌿 مواد اولیه</button><button data-c="prod" class="'+(cat=='prod'?'on':'')+'">🛢 محصولات</button><button data-c="part" class="'+(cat=='part'?'on':'')+'">🔧 قطعات</button></div><div class="panel"><h3>افزودن / به‌روزرسانی</h3><div class="fgrid"><input class="inp" id="iN" placeholder="نام کالا"><input class="inp" type="number" id="iQ" placeholder="موجودی"><input class="inp" id="iU" placeholder="واحد (kg/لیتر/عدد)"><input class="inp" type="number" id="iM" placeholder="حد هشدار"></div><button class="fbtn" id="iAdd">ذخیره</button></div><div class="panel"><h3>موجودی‌ها</h3><div class="tbw"><table><tr><th>کالا</th><th>موجودی</th><th>واحد</th><th>حد هشدار</th><th>وضعیت</th></tr>'+rows+'</table></div></div>';
$$('#main .seg button').forEach(function(b){b.onclick=function(){cat=b.getAttribute('data-c');draw()}});
$('#iAdd').onclick=function(){var n=$('#iN').value.trim();if(!n){toast('نام کالا را وارد کن.',1);return}var inv2=D('pp_inv',[]),ex=null;for(var j=0;j<inv2.length;j++){if(inv2[j].c===cat&&inv2[j].n===n){ex=inv2[j];break}}var it={c:cat,n:n,q:N0($('#iQ').value),u:STR($('#iU').value,'kg'),m:N0($('#iM').value)};if(ex){ex.q=it.q;ex.u=it.u;ex.m=it.m}else inv2.push(it);SV('pp_inv',inv2);toast('انبار به‌روزرسانی شد.');draw()}}
draw()};
...
else{var chk=b.chk?b.chk:CHK.map(function(){return false}),lock=false;for(var k=0;k<CHK.length;k++){if(CHK[k][1]&&!chk[k]){lock=true;break}}
...
if(b.st=='cooling'&&canStart)html+='<div class="fgrid" style="margin-top:10px"><input class="inp" type="number" id="cT" placeholder="دمای کنونی (°C)"></div><button class="fbtn" id="stCool">✅ تأیید دمای مجاز</button>';
...
$('#main').innerHTML=html;
$$('#main .tick').forEach(function(t){t.onchange=function(){var nt=$$('#main .tick');b.chk=CHK.map(function(c,k){return nt[k].checked});SV('pp_batches',bs);draw()}});
var n=$('#nAdd');if(n)n.onclick=function(){var t=N0($('#nT').value);if(!t){toast('تناژ را وارد کن.',1);return}bs.push({id:'B-'+today().replace(/-/g,'').slice(2)+'-'+String(Date.now()).slice(-3),m:$('#nM').value,t:t,op:S.user.name,sh:STR($('#nS').value,'A'),d:today(),st:'created',chk:CHK.map(function(){return false})});SV('pp_batches',bs);toast('Batch ایجاد شد.');draw()};
...
var o=$('#oAdd');if(o)o.onclick=function(){var oil=N0($('#oOil').value),car=N0($('#oCar').value),wr=N0($('#oWire').value);if(!oil&&!car){toast('خروجی را وارد کن.',1);return}b.o2=oil;b.c2=car;b.w2=wr;b.st='done';b.end=now();
var inv=D('pp_inv',[]);function add(n2,q,u){if(!q)return;var f2=null;for(var z=0;z<inv.length;z++){if(inv[z].c==='prod'&&inv[z].n===n2){f2=inv[z];break}}if(f2)f2.q=N0(f2.q)+q;else inv.push({c:'prod',n:n2,q:q,u:u,m:0})}
add('روغن',oil,'لیتر');add('کربن بلک',car,'kg');add('سیم',wr,'kg');SV('pp_inv',inv);toast('خروجی ثبت شد ✅');draw()}}
draw()};
...
RENDER.sales=function(){var inv=D('pp_inv',[]),prods=inv.filter(function(i){return i.c==='prod'&&N0(i.q)>0}),sl=D('pp_sales',[]);
...
var p=null;for(var z=0;z<inv.length;z++){if(inv[z].c==='prod'&&inv[z].n===pn){p=inv[z];break}}if(!p){toast('محصول یافت نشد.',1);return}if(N0(p.q)<q){toast('موجودی کافی نیست.',1);return}
p.q=N0(p.q)-q;SV('pp_inv',inv);sl.push({d:$('#sD').value,cu:STR($('#sC').value,'—'),pr:pn,q:q,p:pr,paid:paid});SV('pp_sales',sl);
var fin=D('pp_fin',[]);fin.push({d:$('#sD').value,t:'in',de:'فروش '+pn+' → '+STR($('#sC').value,'مشتری'),a:paid});SV('pp_fin',fin);toast('فروش ثبت شد ✅');RENDER.sales()}};
...
RENDER.tm=function(){var inv=D('pp_inv',[]),parts=inv.filter(function(i){return i.c==='part'}),arr=D('pp_tm',[]);
...
$('#main').innerHTML=head('تعمیرات و نگهداری','مصرف قطعه → کسر انبار + هزینه مالی')+'<div class="panel"><h3>ثبت تعمیر</h3><div class="fgrid"><input class="inp" type="date" id="tD" value="'+today()+'"><input class="inp" id="tE" value="R-01"><select class="inp" id="tP">'+(parts.length?parts.map(function(p){return'<option>'+esc(p.n)+'</option>'}).join(''):'<option value="">— قطعه‌ای نیست —</option>')+'</select></div><div class="fgrid"><input class="inp" type="number" id="tQ" placeholder="تعداد"><input class="inp" id="tW" placeholder="تعمیرکار"><input class="inp" type="number" id="tC" placeholder="هزینه (ریال)"></div><input class="inp" id="tX" placeholder="شرح کار"><button class="fbtn" id="tAdd">ثبت</button></div><div class="panel"><h3>سابقه</h3><div class="tbw"><table><tr><th>تاریخ</th><th>تجهیز</th><th>قطعه</th><th>تعداد</th><th>نیرو</th><th>هزینه</th></tr>'+rows+'</table></div></div>';
$('#tAdd').onclick=function(){var q=N0($('#tQ').value),c=N0($('#tC').value),pn=$('#tP').value;
if(pn&&q){var f2=null;for(var z=0;z<inv.length;z++){if(inv[z].c==='part'&&inv[z].n===pn){f2=inv[z];break}}if(!f2){toast('قطعه در انبار نیست.',1);return}if(N0(f2.q)<q){toast('موجودی قطعه کافی نیست.',1);return}f2.q=N0(f2.q)-q;SV('pp_inv',inv)}
...
$('#main').innerHTML=head('کاربران و نقش‌ها','نقش هر نفر را انتخاب و ذخیره کن')+'<div class="panel"><div class="tbw"><table><tr><th>نام</th><th>ایمیل</th><th>نقش</th><th>تغییر نقش</th></tr>'+rows+'</table></div><button class="fbtn" id="uSave" style="
